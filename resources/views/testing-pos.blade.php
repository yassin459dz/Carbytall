...
function initApp() {
  const app = {
    ...
    async startWithSampleData() {
      const response = await fetch("{{ asset('data/sample.json')}} ");
...
<body class="bg-blue-gray-50" x-data="initApp()" x-init="initDatabase()">
  ...
  <div class="flex flex-row h-screen antialiased hide-print text-blue-gray-800">
    <!-- left sidebar -->
    <div class="flex flex-row flex-shrink-0 w-auto py-4 pl-4 pr-2">
      <div class="flex flex-col items-center flex-shrink-0 w-20 py-4 bg-cyan-500 rounded-3xl">
        <a href="#"
           class="flex items-center justify-center w-12 h-12 rounded-full bg-cyan-50 text-cyan-700">
        ...
        <ul class="flex flex-col mt-12 space-y-2">
          ...
          <li>
            <a href="#"
               class="flex items-center">
              <span class="flex items-center justify-center w-12 h-12 text-cyan-100 hover:bg-cyan-400 rounded-2xl">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <a
          ...
          target="_blank"
          class="flex items-center justify-center w-10 h-10 mt-auto text-cyan-200 hover:text-cyan-100 focus:outline-none"
        >
    ...
    <div class="flex flex-grow">
      ...
      <div class="flex flex-col w-full h-full py-4 bg-blue-gray-50">
        <div class="relative flex flex-row px-2">
          ...
          <input
            type="text"
            class="w-full h-16 py-4 pl-16 text-lg transition-shadow bg-white shadow rounded-3xl full focus:shadow-2xl focus:outline-none"
          ...
          <div class="h-full px-2 overflow-y-auto">
            ...
            >
              <div class="w-full text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-24 h-24" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div x-show="filteredProducts().length" class="grid grid-cols-4 gap-4 pb-3">
              <template x-for="product in filteredProducts()" :key="product.id">
                <div
                  role="button"
                  class="overflow-hidden transition-shadow bg-white shadow cursor-pointer select-none rounded-2xl hover:shadow-lg"
                  :title="product.name"
                  x-on:click="addToCart(product)"
                >
                  <img :src="product.image" :alt="product.name">
                  <div class="flex px-3 pb-3 -mt-3 text-sm">
                    <p class="flex-grow mr-1 truncate" x-text="product.name"></p>
                    <p class="font-semibold nowrap" x-text="priceFormat(product.price)"></p>
                  </div>
                </div>
              </template>
            </div>
      ...
      <!-- right sidebar -->
      <div class="flex flex-col w-5/12 h-full py-4 pl-2 pr-4 bg-white bg-blue-gray-50">
        <div class="flex flex-col h-full bg-white shadow rounded-3xl">
          <!-- empty cart -->
          <div x-show="cart.length === 0" class="flex flex-col flex-wrap content-center justify-center flex-1 w-full p-4 opacity-25 select-none">
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block h-16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- cart items -->
          <div x-show="cart.length > 0" class="flex flex-col flex-1 overflow-auto">
            <div class="flex justify-center h-16 text-center">
              <div class="relative py-4 pl-8 text-lg text-left">
                <!-- cart icon -->
                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
                <div x-show="getItemsCount() > 0" class="absolute w-5 h-5 p-0 text-xs leading-5 text-center text-white rounded-full bg-cyan-500 -right-2 top-3" x-text="getItemsCount()"></div>
              </div>
              <div class="relative flex-grow px-8 py-4 text-lg text-right">
                <!-- trash button -->
            ...
            <div class="flex-1 w-full px-4 overflow-auto">
              <template x-for="item in cart" :key="item.productId">
                <div class="flex justify-center w-full px-2 py-2 mb-3 rounded-lg select-none bg-blue-gray-50 text-blue-gray-700">
                  <img :src="item.image" alt="" class="w-10 h-10 mr-2 bg-white rounded-lg shadow">
                  <div class="flex-grow">
                    <h5 class="text-sm" x-text="item.name"></h5>
                    <p class="block text-xs" x-text="priceFormat(item.price)"></p>
                  </div>
                  <div class="py-1">
                    <div class="grid grid-cols-3 gap-2 ml-2 w-28">
                      <button x-on:click="addQty(item, -1)" class="py-1 text-center text-white rounded-lg bg-blue-gray-600 hover:bg-blue-gray-700 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-3 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                        </svg>
                      </button>
                      <input x-model.number="item.qty" type="text" class="text-sm text-center bg-white rounded-lg shadow focus:outline-none focus:shadow-lg">
                      <button x-on:click="addQty(item, 1)" class="py-1 text-center text-white rounded-lg bg-blue-gray-600 hover:bg-blue-gray-700 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-3 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="w-full h-auto px-4 pt-3 pb-4 text-center select-none">
            <div class="flex mb-3 text-lg font-semibold text-blue-gray-700">
              <div>TOTAL</div>
              <div class="w-full text-right" x-text="priceFormat(getTotalPrice())"></div>
            </div>
            <div class="px-3 pt-2 pb-3 mb-3 rounded-lg text-blue-gray-700 bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                <div class="flex-grow text-left">CASH</div>
                <div class="flex text-right">
                  <div class="mr-2">Rp</div>
                  <input x-bind:value="numberFormat(cash)" x-on:keyup="updateCash($event.target.value)" type="text" class="px-2 text-right bg-white rounded-lg shadow w-28 focus:bg-white focus:shadow-lg focus:outline-none">
                </div>
              </div>
              <hr class="my-2">
              <div class="grid grid-cols-3 gap-2 mt-2">
                <template x-for="money in moneys">
                  <button x-on:click="addCash(money)" class="inline-block px-2 py-1 text-sm bg-white rounded-lg shadow hover:shadow-lg focus:outline-none">+<span x-text="numberFormat(money)"></span></button>
                </template>
              </div>
            </div>
            <div
              x-show="change > 0"
              class="flex px-3 py-2 mb-3 text-lg font-semibold rounded-lg bg-cyan-50 text-blue-gray-700"
            >
              <div class="text-cyan-800">CHANGE</div>
              <div
                class="flex-grow text-right text-cyan-600"
                x-text="priceFormat(change)">
              </div>
            </div>
            <div
              x-show="change < 0"
              class="flex px-3 py-2 mb-3 text-lg font-semibold bg-pink-100 rounded-lg text-blue-gray-700"
            >
              <div
                class="flex-grow text-right text-pink-600"
                x-text="priceFormat(change)">
              </div>
            </div>
            <div
              x-show="change == 0 && cart.length > 0"
              class="flex justify-center px-3 py-2 mb-3 text-lg font-semibold rounded-lg bg-cyan-50 text-cyan-700"
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              class="w-full py-3 text-lg text-white rounded-2xl focus:outline-none"
              x-bind:class="{
    ...
    <!-- modal first time -->
    <div x-show="firstTime" class="fixed top-0 left-0 z-10 flex flex-wrap content-center justify-center w-full h-screen p-24 glass">
      <div class="p-8 bg-white shadow-xl w-96 rounded-3xl">
        <div class="text-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="inline-block" width="123.3" height="123.233" viewBox="0 0 32.623 32.605">
            <path d="M15.612 0c-.36.003-.705.01-1.03.021C8.657.223 5.742 1.123 3.4 3.472.714 6.166-.145 9.758.019 17.607c.137 6.52.965 9.271 3.542 11.768 1.31 1.269 2.658 2 4.73 2.57.846.232 2.73.547 3.56.596.36.021 2.336.048 4.392.06 3.162.018 4.031-.016 5.63-.221 3.915-.504 6.43-1.778 8.234-4.173 1.806-2.396 2.514-5.731 2.516-11.846.001-4.407-.42-7.59-1.278-9.643-1.463-3.501-4.183-5.53-8.394-6.258-1.634-.283-4.823-.475-7.339-.46z" fill="#fff"/><path d="M16.202 13.758c-.056 0-.11 0-.16.003-.926.031-1.
 0-.16.003-.925.031-1.38.172-1.746.539-.42.42-.554.981-.528 2.207.02 1.018.15 1.448.553 1.838.204.198.415.312.738.4.132.037.426.086.556.094.056.003.365.007.686.009.494.003.63-.002.88-.034.61-.08 1.003-.278 1.285-.652.282-.374.393-.895.393-1.85 0-.688-.066-1.185-.2-1.506-.228-.547-.653-.863-1.31-.977a7.91 7.91 0 00-1.146-.072zM22.468 13.736c-.056 0-.11.001-.161.003-.925.032-1.38.172-1.746.54-.42.42-.554.98-.528 2.207.021 1.018.15 1.447.553 1.837.205.198.415.313.739.401.132.037.426.086.556.094.056.003.364.007.685.009.494.003.63-.002.88-.035.611-.078 1.004-.277 1.285-.651.282-.375.393-.895.393-1.85 0-.688-.065-1.185-.2-1.506-.228-.547-.653-.864-1.31-.977a7.91 7.91 0 00-1.146-.072z" fill="#00dace"/><path d="M9.937 13.736c-.056 0-.11.001-.161.003-.925.032-1.38.172-1.746.54-.42.42-.554.98-.528 2.207.021 1.018.15 1.447.553 1.837.204.198.415.313.738.401.133.037.427.086.556.094.056.003.365.007.686.009.494.003.63-.002.88-.035.61-.078 1.003-.277 1.285-.651.282-.375.393-.895.393-1.85 0-.688-.066-1.185-.2-1.506-.228-.547-.653-.864-1.31-.977a7.91 7.91 0 00-1.146-.072zM16.202 7.59c-.056 0-.11 0-.16.002-.926.032-1.38.172-1.747.54-.42.42-.553.98-.528 2.206.022 1.019.151 1.448.553 1.838.205.198.415.312.739.401.132.037.426.086.556.093.056.003.365.007.686.01.494.002.63-.003.879-.035.611-.079 1.004-.278 1.286-.652.282-.374.392-.895.393-1.85 0-.688-.066-1.185-.2-1.505-.228-.547-.653-.864-1.31-.978a7.91 7.91 0 00-1.147-.071z" fill="#00bcd4"/></g>
          </svg>
          <h3 class="mb-8 text-2xl text-center">FIRST TIME?</h3>
        </div>
        <div class="text-left">
          <button x-on:click="startWithSampleData()" class="w-full px-4 py-4 mb-3 text-left text-white rounded-xl bg-blue-gray-500 focus:outline-none hover:bg-cyan-400">
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-6 h-6 mr-2 -mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-2m-4-1v8m0 0l3-3m-3 3L9 8m-5 5h2.586a1 1 0 01.707.293l2.414 2.414a1 1 0 00.707.293h3.172a1 1 0 00.707-.293l2.414-2.414a1 1 0 01.707-.293H20" />
            </svg>
            LOAD SAMPLE DATA
          </button>
          <button x-on:click="startBlank()" class="w-full px-4 py-4 text-left text-white rounded-xl bg-blue-gray-500 focus:outline-none hover:bg-teal-400">
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-6 h-6 mr-2 -mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <div
        x-show="isShowModalReceipt"
        class="fixed top-0 left-0 z-0 w-full h-screen glass" x-on:click="closeModalReceipt()"
      ...
      <div
        x-show="isShowModalReceipt"
        class="z-10 overflow-hidden bg-white shadow-xl w-96 rounded-3xl"
        x-transition:enter="transition ease-out duration-100"
      ...
      >
        <div id="receipt-content" class="w-full p-6 overflow-auto text-sm text-left">
          <div class="text-center">
            <img src="img/receipt-logo.png" alt="Tailwind POS" class="inline-block w-8 h-8 mb-3">
        ...
        <div class="w-full p-4">
          <button class="w-full px-4 py-3 text-lg text-white bg-cyan-500 rounded-2xl focus:outline-none" x-on:click="printAndProceed()">PROCEED</button>
        </div>
      </div>
    </div>
  </div>
